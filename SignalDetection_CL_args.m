...
%function [] = SignalDetection_Mar23_CL_args(putatVarStart, putatVarStop)
...
%coveragefiles = dir('/projects/mgc/Project_2/cortex_var_runs/validating_coverage/covgmetrics_output/CNTNAP4_trios/*.covg_metrics_output');
%coveragefiles = dir('/projects/mgc/Project_2/cortex_var_runs/validating_coverage/running_all_gene_loci_Dec2019/output/biobank/chr7_17930076_17930142_SNX13/*.covg_metrics_output');
coveragefiles = dir(filesDir);
NumFiles=size(coveragefiles);


% create the output filer and print the headers
%OutputFile = fopen('matlab_outputs/chr7_17930076_17930142_SNX13_biobank','w');
...
for file = 1:NumFiles(1)
   ...
   %elseif (((str2num(FirstCoord{2})+TheChange(1)-1<PutativeVariantStart) & (str2num(FirstCoord{2})+TheChange(2)-1>PutativeVariantStop)) | (str2num(FirstCoord{2})+TheChange(1)-1>PutativeVariantStop)) | (str2num(FirstCoord{2})+TheChange(2)-1<PutativeVariantStart)
%    elseif ( ((str2num(FirstCoord{2})+TheChange(1)-1>PutativeVariantStart) & (str2num(FirstCoord{2})+TheChange(2)-1<PutativeVariantStop)) & (((str2num(FirstCoord{2})+TheChange(2)) - (str2num(FirstCoord{2})+TheChange(1))) > ((PutativeVariantStop-PutativeVariantStart)/2)) )
...
%fprintf('%s%d\n','CountIndividualsWithPutativeVariant=',CountIndividualsWithPutativeVariant);
end
